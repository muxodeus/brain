...
import UnifilarCanvas from "./UnifilarCanvas";
...
// Jerarquía industrial con ~20 medidores, 5 niveles, incluyendo panel banco de capacitores y campo FV
...
// Utilidad: pesos normalizados para que hermanos sumen 100% del padre
...
// Crea un mapa de métricas completo, respetando reglas de capacitores y solares
...
const assign = (nodes: NodeItem[], parentKW: number, parentKVAR: number) => {
  ...
  // pesos independientes para kW y kVAR
  ...
  nodes.forEach((n, i) => {
    ...
    // Reglas especiales
    if (n.name.toLowerCase().includes("capacitor")) {
      childKW = 0; // banco de capacitores solo aporta kVAR
    }
    if (n.name.toLowerCase().includes("inversor")) {
      childKVAR = 0; // inversor FV solo aporta kW
    }
...
export default function UnifilarPage() {
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      const rootKW = +(800 + Math.random() * 400).toFixed(1); // 800–1200 kW
      const rootKVAR = +(300 + Math.random() * 300).toFixed(1); // 300–600 kVAR
...
const pctKWRoot =
  selectedNode.id === tree[0].id
    ? "100"
    : root.kW === 0
    ? "0"
    : ((m.kW / root.kW) * 100).toFixed(1);
...
const pctKVARRoot =
  selectedNode.id === tree[0].id
    ...
    : root.kVAR === 0
    ? "0"
    : ((m.kVAR / root.kVAR) * 100).toFixed(1);
...
const pctKVARParent =
    ...
    return (
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "12px" }}>
        <div style={{ background: "#0b1220", border: "1px solid #334155", borderRadius: 8, padding: 12 }}>
          ...
          <div style={{ fontSize: 20, fontWeight: 600, color: "#93c5fd" }}>
            {m.kW.toFixed(1)} kW
          </div>
        ...
        <div style={{ background: "#0b1220", border: "1px solid #334155", borderRadius: 8, padding: 12 }}>
          ...
          <div style={{ fontSize: 20, fontWeight: 600, color: "#fbbf24" }}>
            {m.kVAR.toFixed(1)} kVAR
          </div>
          <div style={{ fontSize: 12, color: "#9ca3af" }}>Contribución</div>
  ...
  return (
    <div style={{ display: "flex", height: "100vh" }}>
      ...
      >
        {/* Mitad superior: jerarquía */}
        <div style={{ flex: 1, overflowY: "auto", padding: "1rem" }}>
          <h3 style={{ marginBottom: "0.75rem" }}>Jerarquía visual</h3>
          <HierarchyPanel
            tree={tree}
      ...
      {/* Columna derecha 70%: canvas + panel de insights detallados */}
